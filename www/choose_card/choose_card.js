...
.controller('ChooseCardCtrl', ["$scope", "$location", "fmaLocalStorage", "$http", "fmaSharedState", "$rootScope", "$timeout", "$q", "$route", "fmaCartHelper", "$routeParams",
function($scope, $location, fmaLocalStorage, $http, fmaSharedState, $rootScope, $timeout, $q, $route, fmaCartHelper, $routeParams) {
  var mainViewObj = $('#main_view_container');
  if (fmaSharedState.testModeEnabled) {
    alert('Warning-- you are using the sandbox.');
  }

  var setupAfterTokenFetched = function() {
  ...
  if (fmaSharedState.fake_token) {
    ...
    $http({
      ...
      $scope.cardsLoading = false;
      $scope.rawAccessToken = response.data.access_token;
      fmaLocalStorage.setObjectWithExpirationSeconds(
    ...
    // Rely on this to route us back if we don't have a token.
    setupAfterTokenFetched();
  }
